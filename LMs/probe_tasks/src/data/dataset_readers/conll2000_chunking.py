...
class Conll2000ChunkingDatasetReader(TaggingDatasetReader):
    """
    ...
    def _read_dataset(self,
        ...
        learn_vocab: return only the pos tag for learning a vocabulary
        shuffle_rng: random seed for shuffling order of instances, suggested
        """
        if count_only:
            logger.info("Counting chunking instances in dataset at: %s", file_path)
        else:
            logger.info("Reading chunking data from dataset at: %s", file_path)
