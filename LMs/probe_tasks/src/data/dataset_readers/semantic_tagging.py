...
def parse_sentence(sentence: str):
    ...
    for line_idx, line in enumerate(lines):
        ...
        for word in filter(None, re.split(r"[~\s]+", line.split("\t")[0])):  # type: ignore
...
class SemanticTaggingDatasetReader(TaggingDatasetReader):
    """
    ...
    This DatasetReader is used to process the semantic tagging data.
    ...
    def _read_dataset(self,
        ...
        shuffle_rng: random seed for shuffling instance orders
        """
        if count_only:
            logger.info("Counting semantic tagging instances in dataset at: %s", file_path)
        else:
            logger.info("Reading semantic tagging data from dataset at: %s", file_path)
